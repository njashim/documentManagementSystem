...
namespace BusinessLayer.Tests
{
    ...
    public class DocumentServiceTests
    {
        ...
        public async Task AddDocumentAsync_ShouldMapAndCallRepository()
        {
            ...
            var fileContent = Encoding.UTF8.GetBytes("This is the content of the document.");

            // Mock für das Mapping und Repository
            _mapperMock.Setup(m => m.Map<Document>(model)).Returns(entity);

            // Act
            await _service.AddDocumentAsync(model, fileContent);  // Übergebe das fileContent
